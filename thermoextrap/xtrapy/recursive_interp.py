...
class RecursiveInterp:
  ...
  def __init__(self, model_cls, coefs, edgeB, maxOrder=1, errTol=0.01):
    self.model_cls = model_cls #The model CLASS used for interpolation, like InterpModel
    self.coefs = coefs #Coefs object describing how derivatives will be calculated
    self.states = [] #List of ExtrapModel objects sharing same Coefs but different Data
  ...
  def getData(self, B):
    ...
    data = Data(udata, xdata, order=self.maxOrder)
    return data

  def recursiveTrain(self, B1, B2, data1=None, data2=None,
    ...
    extrap1 = ExtrapModel(alpha0=B1, data=data1, coefs=self.coefs, order=self.maxOrder)
    extrap2 = ExtrapModel(alpha0=B2, data=data2, coefs=self.coefs, order=self.maxOrder)
    ...
    bootErr = this_model.resample(100).predict(Bvals, order=self.maxOrder).std('rep')
    ...
    relErr = relErr.where(relErr != np.inf).fillna(0.0) #Replaces Inf with NaN, then NaN to 0
...
training happens just on those without adding points.
  ...
  def checkPolynomialConsistency(self, doPlot=False):
    ...
    for i, aset in enumerate(edgeSets):
      ...
      reg1Err = reg1Model.resample(100).xcoefs(order=self.maxOrder).std('rep')
      reg2Model = self.model_cls((self.states[aset[1]], self.states[aset[2]]))
      reg2Coeffs = reg2Model.xcoefs(order=self.maxOrder)
      reg2Err = reg2Model.resample(100).xcoefs(order=self.maxOrder).std('rep')
      ...
      p12 = stats.norm.cdf(abs(z12)) - stats.norm.cdf(-abs(z12)) #Null hypothesis coefficients different
      ...
      fullErr = fullModel.resample(100).xcoefs(order=self.maxOrder).std('rep')
