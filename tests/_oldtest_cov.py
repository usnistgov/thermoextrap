...
def test_vals(nrec, moments, weighted):
    ...
    s = central.CentralMomentsCov.zeros(mom=moments)
    s.push_vals((x0, x1), w=w)
    np.testing.assert_allclose(s.data, data)

    s = central.CentralMomentsCov.from_vals(x=(x0, x1), w=w, mom=moments)
    np.testing.assert_allclose(s.data, data)
...
@pytest.mark.parametrize("weighted", [False, True])
def test_CentralMoments_stats(nrec, moments, weighted):
    # unweighted
    w, x0, x1 = _get_data_single(nrec, weighted)
    ...
    # factory
    s = central.CentralMomentsCov.from_datas(datas, mom=moments)
    np.testing.assert_allclose(s.data, data)
    ...
    # addition
    S = [central.CentralMomentsCov.from_data(d, mom=moments) for d in datas]
    out = S[0]
    for s in S[1:]:
        out = out + s
    np.testing.assert_allclose(out.data, data)

    out = sum(S, central.CentralMomentsCov.zeros(mom=moments))
    np.testing.assert_allclose(out.data, data)

    out = central.CentralMomentsCov.zeros(mom=moments)
    for s in S:
        out += s
    np.testing.assert_allclose(out.data, data)
    ...
    # imul
    out = central.CentralMomentsCov.from_vals((x0s[0], x1s[0]), ws[0], mom=moments)
    out *= 2
...
def test_vec_vals(dshape, axis, moments, weighted, broadcast):
    ...
    for w in (wt, ws):
        ...
        s = central.CentralMomentsCov.zeros(shape=shape, mom=moments)
        # push_vals
        s.push_vals((x0, x1), w=w, axis=axis, broadcast=broadcast)
        np.testing.assert_allclose(s.data, data)

        # from vals
        s = central.CentralMomentsCov.from_vals((x0, x1), w, mom=moments, axis=axis, broadcast=broadcast)
        np.testing.assert_allclose(s.data, data)
        ...
        if x1b is not None:
            ...
            s = central.CentralMomentsCov.from_vals((x0, x1b), w, mom=moments, axis=axis, broadcast=False)
            np.testing.assert_allclose(s.data, data)
...
def test_Vec_stats(dshape, axis, moments, weighted):
    ...
    for w in (wt, ws):
        ...
        # factory
        s = central.CentralMomentsCov.from_datas(datas, mom=moments, axis=0)
        np.testing.assert_allclose(s.data, data)

        # pushs
        s = central.CentralMomentsCov.zeros(mom=moments, shape=shape)
