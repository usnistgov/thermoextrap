...
class StatsAccumBase(object):
    ...
    def _get_target_shape(self, shape=None, nrec=None, axis=None, data=False):
        """
    ...
    def check_weights(self, w, target, axis=0):
        if w is None:
            w = 1.0
        return self._verify_value(
            w, target=target, axis=axis,
            broadcast=True, expand=True,
            shape_flat=self._shape_flat
        )

    def check_val(self, x, target, broadcast=False):
        return self._verify_value(
            x, target=target, axis=axis,
            broadcast=broadcast, expand=False,
            shape_flat=self._shape_flat,
        )
    ...
    def check_vals(self, x, target, axis=0, broadcast=False):
        return self._verify_value(
            x, target=target, axis=axis,
            broadcast=broadcast, expand=broadcast,
            shape_flat=self._shape_flat,
        )
    ...
    def check_data(self, data):
        return self._verify_value(
            data, target=self._data_shape,
            axis=None,
            broadcast=False, expand=False,
            shape_flat=self._data_flat_shape
        )[0]


    def check_datas(self, datas, axis=0):
        datas = self._asarray(datas)
        return self._verify_value(
            datas, target=_shape_insert_axis(self._data_shape, axis, datas.shape[0]),
            axis=axis, broadcast=False, expand=False,
            shape_flat=self._data_flat_shape
        )[0]
...
class _StatsAccumMixin(object):
    def push_val(self, x, w=None):
        xr, target = self.check_val(x, target=self.shape)
        wr = self.check_weight(w, target=target)
        self._push.val(self._data_flat, wr, xr)

    def push_vals(self, x, w=None, axis=0):
        target_shape = self._get_target_shape(x.shape[axis], )
        xr, target = self.check_vals(x, axis=axis, target=self.shape)
        wr = self.check_weights(w, target=target, axis=axis)
        self._push.vals(self._data_flat, wr, xr)

    def push_stat(self, a, v=0.0, w=None):
        ar, target = self.check_val(a, target=self.shape)
        vr = self.check_val(v, target=self._shape_var,
    ...
    def push_stats(self, a, v=0.0, w=None, axis=0):
        ar, target = self.check_vals(a, target=self.shape, axis=axis)
